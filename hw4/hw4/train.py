...
High level training & eval looping structure:

```
model = load_lora_policy_model_and_tokenizer(...)  # frozen base LM + trainable LoRA policy
task: hw4.tasks.base.Task = ...  # e.g. MathHardTask
algo: hw4.rl.GRPO = ...  # update-rule object; does not own model/optimizer

baseline_stats = run_eval_for_task(...)
for step in cfg.steps:
  examples = task.sample_train_batch(cfg.batch_size)

  # For each prompt, sample group_size completions using the current policy.
  # The rollout caches old policy logprobs, reference logprobs, and completion masks.
  rollout = sampler.rollout(model, group_size, ...)

  # Score each completion with the task reward. Then normalize rewards within
  # that prompt's group to form group-relative advantages. This uses the other
  # completions for the same prompt as a local baseline, avoiding a separate
  # value network.
  rollout_batch = RolloutBatch(..., rewards=rewards, advantages=advantages)

  # Applies RL updates to LoRA params
  algo.update(model=model, optimizer=optimizer, rollout=rollout_batch, ...)

  if step % N == 0:
    periodic_stats = run_eval_for_task(...)

final_stats = run_eval_for_task(...)
```
"""
