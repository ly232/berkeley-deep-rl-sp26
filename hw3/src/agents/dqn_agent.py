...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.0) -> int:
        ...
        """
        observation = ptu.from_numpy(np.asarray(observation)).unsqueeze(
            0
        )  # unsqueeze to add batch dimension

        # TODO(DONE)(Section 2.4): get the action from the critic using an epsilon-greedy strategy
        with torch.no_grad():
            if np.random.random() < epsilon:
                return int(np.random.randint(self.num_actions))
            else:
                return torch.argmax(self.critic(observation), dim=1).item()
        # ENDTODO

    def update_critic(
        self,
        obs: torch.Tensor,  # (batch_size, *obs_shape)
        action: torch.Tensor,  # (batch_size,)
        reward: torch.Tensor,  # (batch_size,)
        next_obs: torch.Tensor,  # (batch_size, *obs_shape)
        ...
        with torch.no_grad():
            # TODO(done)(Section 2.4): compute target values
            ...
            if self.use_double_q:
                ...
                next_action = torch.argmax(
                    self.critic(next_obs), dim=1
                )  # (batch_size,)
            else:
                next_action = torch.argmax(next_qa_values, dim=1)  # (batch_size,)

            next_q_values = torch.gather(
                input=next_qa_values, dim=1, index=next_action.unsqueeze(1)
            ).squeeze(1)
        ...
        # TODO(done)(Section 2.4): train the critic with the target values
        qa_values = self.critic(obs)  # (batch_size, num_actions)
        q_values = torch.gather(
            input=qa_values, dim=1, index=action.unsqueeze(1)
        ).squeeze(1)
    ...
    def update(
        ...
        """
        # TODO(done)(Section 2.4): update the critic, and the target if needed
